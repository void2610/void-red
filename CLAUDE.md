...
シナリオ / PlayMode ランナー / CI の使い分けと決定性パターンは [`Docs/test-strategy.md`](./Docs/test-strategy.md) を参照。要点:

- **検証は `[LiminalScenario]` (`Assets/Scripts/Debug/*Scenarios.cs`) として残す**。手で `liminal exec` を連打して終わらせない。バグ修正なら再現手順を、新機能なら happy path と境界値をシナリオにする
