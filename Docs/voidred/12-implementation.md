...
- Canvas の参照解像度は 1920x1080 だが、**`Window()` で作る全画面窓の論理サイズは 1067x600**。窓の中の `Place` は
  この座標系 (±533, ±300) で考える。範囲を超えると画面外に出て何も見えなくなる (洗礼の説明文で踏んだ)。
- 旧資産の見出しには**画像に文字が焼き込まれている**ものがある (`result_bg` の「オークション結果」)。
  テキスト置換では消せないので、新しい見出しは画像の文字と重ならない位置に置く。
- 参加者アイコンは `Canvas/ParticipantBar` の下に生成され、**`AuctionSceneView` の子ではない**。
  `GetComponentsInChildren` では拾えないので `AuctionSceneView.Icons` から引く。
- **操作要素は左と中央下に集める** (ラフ `images/ui-bid-screen.png` の意図)。右は立ち絵の領域で、
  ボタンを置くと立ち絵の裏に回り込んで読めなくなる。下端は参加者バーが占める。
- レイアウトを変えたら**必ず実画面を撮って確認する**。ボタンは名前で引けるので、隠れていても E2E は通ってしまう。
  `UI/Screenshot` にパスを渡して撮り、フェーズごとに見る。
- `CompetitionView` は旧資産の作りで**ルートの localScale が極端に小さく、子は巨大な sizeDelta で釣り合わせている**
  (`Background` は rect 1067x600 でも world では幅 18)。この中の要素を全画面に広げようとしても効かないので、
  暗幕を足すなら `CompetitionView` の外 (Canvas 直下) に置く。競合中に画面左端へ前フェーズが僅かに覗くのはこれが原因。
